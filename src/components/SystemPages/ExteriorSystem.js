...
function ExteriorSystem() {

   const problems = useOutletContext();

   const exteriorProblemList = problems.filter(item => item.system === 'Экстерьер');
            ...
            <div className='problem__block'>
               <ul className='problem__list'>
                  <li className='problem__item'>
                     {exteriorProblemList.sort((a, b) => parseInt(b.defect_qty) - parseInt(a.defect_qty)).slice(0, 3).map((problem, index) => (
